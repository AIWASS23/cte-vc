...
def avaliar_modelo(best_path, val_dir, imgsz):
    ...
    spec = tn / (tn + fp)
    return {'acuracia': acc, 'precisao': prec, 'revocacao': rec, 'f1': f1, 'especificidade': spec}
...
for exp in experimentos:
    ...
    best_path = os.path.join('runs_hiper_binario', exp['nome'], 'weights', 'best.pt')
    ...
    print(f"  Acurácia: {metricas['acuracia']:.4f} | F1: {metricas['f1']:.4f} | Espec: {metricas['especificidade']:.4f}")
...
print("\n" + "=" * 80)
print("TABELA COMPARATIVA DE HIPERPARÂMETROS - BINÁRIO")
print("=" * 80)
print(df[['experimento', 'modelo', 'epochs', 'batch', 'lr0', 'acuracia', 'precisao', 'revocacao', 'f1', 'especificidade']].to_string(index=False))

df.to_csv('resultados_hiperparametros_binario.csv', index=False)
print("\nResultados salvos em resultados_hiperparametros_binario.csv")
...
specs = df['especificidade'].values
...
bars3 = ax.bar(x + width, specs, width, label='Especificidade', color='#9b59b6', alpha=0.8)

ax.set_xlabel('Experimento')
ax.set_ylabel('Valor da Métrica')
ax.set_title('Comparação de Hiperparâmetros - Classificação Binária')
ax.set_xticks(x)
...
plt.tight_layout()
plt.savefig('yolo_binaria_comparacao_hiperparametros.png', dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('yolo_binaria_variacao_hiperparametros.png', dpi=150, bbox_inches='tight')
plt.show()
